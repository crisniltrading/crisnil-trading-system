...
function switchTab(tabId) {
    ...
    if (tabId === 'expiry-tracker') {
        // Load expiry tracker dashboard
        if (typeof loadExpiryDashboard === 'function') {
            loadExpiryDashboard();
        }
    }
    if (tabId === 'analytics') {
...
async function handleAddProduct(e) {
    ...
    if (batchNumber && expiryDate) {
        productData.batchInfo = [{
            ...
            quantity: parseInt(formData.get('stock')), // Use initial stock as batch quantity
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (imageFile && imageFile.size > 0) {
                console.log('📤 Converting and uploading image for new product:', newProductId);
...
function updateStatsGrid(data) {
    ...
    // Enhanced admin stats with growth indicators
    const stats = [
        {
    ...
    statsGrid.innerHTML = stats.map(stat => `
        <div class="stat-card stat-card-enhanced">
            ...
            <div class="stat-content">
                ...
                <div class="stat-label">${stat.label}</div>
                ${stat.growth !== null ? `
                    <div class="stat-growth ${stat.growth >= 0 ? 'growth-positive' : 'growth-negative'}">
...
async function createDirectPromo() {
    ...
    products.forEach(product => {
        ...
        productCheckboxesHTML += `
            <label style="display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem; background: var(--background); border-radius: 4px; cursor: pointer;">
                <input type="checkbox" name="products" value="${product._id}" style="cursor: pointer;">
                <span style="flex: 1;">${product.name}</span>
                <span style="color: var(--text-muted); font-size: 0.875rem;">₱${product.price.toFixed(2)}</span>
            </label>
        `;
    });
...
async function loadPromotions() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Separate active, scheduled, and expired
            const now = new Date();
            ...
            console.log('Inactive promotions:', inactive.length);
            ...
            const totalDiscount = active.reduce((sum, p) => {
            ...
            // Update lists - render to both overview and promotions tab
            renderPromotionsList(active, 'overviewPromotionsList'); // Overview section
            renderPromotionsList(active, 'activePromotionsList'); // Promotions tab
            renderPromotionsList(inactive, 'scheduledExpiredPromotionsList', true);
...
async function togglePromotion(promoId, activate) {
    ...
    try {
        ...
        if (response.ok) {
            showToast(`Promotion ${activate ? 'activated' : 'deactivated'} successfully!`, 'success');
            loadPromotions();
        } else {
...
function displayTransactionModal(transaction) {
    // Create detailed view modal with proper CSS classes
    const modal = document.createElement('div');
    modal.className = 'modal-overlay';
    modal.style.cssText = `
    ...
    modal.innerHTML = `
        <div class="modal-content card" style="max-width: 700px; width: 90%; max-height: 85%; overflow-y: auto; margin: 2rem; background: var(--surface); color: var(--text);">
            <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; padding-bottom: 1rem; border-bottom: 1px solid var(--border);">
                <h2 style="margin: 0; color: var(--text); display: flex; align-items: center; gap: 0.5rem;">
                    ...
                    ${isClient ? 'Order Details' : 'Transaction Details'}
                </h2>
            ...
            <div class="modal-body" style="display: grid; gap: 1.5rem;">
                ...
                <div class="transaction-overview" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; padding: 1rem; background: var(--background-alt); border-radius: var(--radius); border-left: 4px solid var(--primary);">
                    <div>
                        <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">${isClient ? 'Order ID' : 'Transaction ID'}</div>
                        <div style="font-weight: 600; color: var(--text);">${transaction.orderId || transaction.id}</div>
                    </div>
                    <div>
                        <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Amount</div>
                        ${transaction.totalDiscount && transaction.totalDiscount > 0 ? `
                            <div style="font-size: 0.9rem; color: var(--text-muted); text-decoration: line-through;">
                                ₱${((transaction.amount || 0) + transaction.totalDiscount).toLocaleString()}
                            </div>
                            <div style="font-size: 0.85rem; color: var(--success); margin-bottom: 0.25rem;">
                                <i class="fas fa-tag"></i> -₱${transaction.totalDiscount.toLocaleString()}
                            </div>
                        ` : ''}
                        <div style="font-weight: 700; font-size: 1.25rem; color: var(--primary);">₱${(transaction.amount || 0).toLocaleString()}</div>
                    </div>
                    <div>
                        <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Status</div>
                        <span class="badge ${getTransactionStatusBadge(transaction.status)}">${transaction.status.toUpperCase()}</span>
                    </div>
                    <div>
                        <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Date</div>
                        <div style="font-weight: 500; color: var(--text);">${new Date(transaction.date).toLocaleDateString()}</div>
                    </div>
                </div>

                <!-- Customer Information (Admin/Staff only) -->
                ${!isClient ? `
                <div class="customer-info">
                    <h4 style="margin-bottom: 1rem; color: var(--text); display: flex; align-items: center; gap: 0.5rem;">
                        <i class="fas fa-user" style="color: var(--info);"></i>
                        Customer Information
                    </h4>
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Customer Name</div>
                            <div style="font-weight: 500; color: var(--text);">${transaction.customer}</div>
                        </div>
                        ${transaction.customerEmail ? `
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Email</div>
                            <div style="font-weight: 500; color: var(--text);">${transaction.customerEmail}</div>
                        </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="payment-info">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Payment Method</div>
                            <div style="font-weight: 500; color: var(--text);">${formatPaymentMethod(transaction.paymentMethod)}</div>
                        </div>
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Reference</div>
                            <div style="font-weight: 500; color: var(--text); font-family: monospace;">${transaction.reference}</div>
                        </div>
                        ${!isClient ? `
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Transaction Fee</div>
                            <div style="font-weight: 500; color: var(--warning);">₱${(transaction.fee || 0).toLocaleString()}</div>
                        </div>
                        <div>
                            <div style="font-size: 0.875rem; color: var(--text-muted); margin-bottom: 0.25rem;">Net Amount</div>
                            <div style="font-weight: 600; color: ${(transaction.netAmount || 0) >= 0 ? 'var(--success)' : 'var(--error)'};">₱${(transaction.netAmount || 0).toLocaleString()}</div>
                        </div>
                        ` : ''}
                    </div>
                </div>
                    ...
                    <div style="background: var(--background-alt); border-radius: var(--radius); overflow: hidden;">
                        ${transaction.items.map((item, index) => `
                            <div style="padding: 1rem; ${index > 0 ? 'border-top: 1px solid var(--border);' : ''} display: flex; justify-content: space-between; align-items: center;">
                                ...
                                <div style="text-align: right;">
                                    <div style="font-weight: 600; color: var(--text);">₱${((item.price || 0) * (item.quantity || 0)).toLocaleString()}</div>
                                    <div style="font-size: 0.875rem; color: var(--text-muted);">₱${(item.price || 0).toLocaleString()} each</div>
                                </div>
                            </div>
                        `).join('')}
                    </div>
                ...
                <!-- Action Buttons -->
                <div class="modal-actions" style="display: flex; gap: 1rem; justify-content: flex-end; padding-top: 1rem; border-top: 1px solid var(--border);">
                    ${!isClient && transaction.status === 'completed' ? `
                        <button class="btn btn-warning" onclick="initiateRefund('${transaction.id}')" title="Process refund">
                            <i class="fas fa-undo"></i> Process Refund
                        </button>
                    ` : ''}
                    ${transaction.orderId ? `
                        <button class="btn btn-info" onclick="trackOrder('${transaction.orderId}')" title="Track order">
                            <i class="fas fa-truck"></i> Track Order
                        </button>
                    ` : ''}
                    <button class="btn btn-secondary" onclick="this.closest('.modal-overlay').remove()">
